...
class Embed(OrginalEmbed):
    ...
    def send_to_webhook(self, message:str = None) -> requests.models.Response:
        send_message_to_webhook(self.webhook_url, message=message, embeds=[self.to_dict()])

def send_message_to_webhook(webhook_url:str, message:str, embeds:list[dict] = None) -> requests.models.Response:
    if webhook_url is None:
        raise ValueError("No webhook URL provided")
    payload = json.dumps({"content": message, "embeds": embeds})
    return send_post_request(webhook_url, payload)
